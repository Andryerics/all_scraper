...
def SetFacebookSite(fbobj, set_to):
    ...
    if set_to not in site_list:
        raise exceptions.FacebookError('Facebook Site tidak valid!!')
    ...
    if c is None:
        raise exceptions.FacebookError('Impossible de trouver le formulaire de préférences sur la page.')

    d = {i.get('name'): i.get('value') for i in c.findAll('input')}  # Data
    e = c.findAll('input', attrs={'name': 'basic_site_devices'})
    
    if not e or site_list.index(set_to.lower()) >= len(e):
        raise exceptions.FacebookError(f"Impossible de trouver l'option '{set_to}' sur la page.")
...
def SetLanguage(fbobj, locale):
    ...
    if c is None:
        raise exceptions.FacebookError('Locale tidak valid, pastikan locale yang anda massukan sudah benar :)')
...
def SwitchMode(fbobj, set_to):
    ...
    if set_to.lower() == 'basic':
        ...
        if upsell is not None:
            ...
            if form_upsell is None:
                raise exceptions.FacebookError("Impossible de trouver le formulaire d'upsell.")

            data_upsell = {
                'fb_dtsg': form_upsell.find('input', attrs={'name': 'fb_dtsg'}).get('value'),
                'jazoest': form_upsell.find('input', attrs={'name': 'jazoest'}).get('value'),
                'data_pack_selection': 'upgrade_to_paid'
            }
...
def ChangePassword(fbobj, old_pass, new_pass, keep_session=False):
    ...
    if c is not None:
        ...
        if '/settings/security/password' in e.url:
            ...
            if form is None:
                raise exceptions.FacebookError("Impossible de trouver le formulaire de validation après le changement de mot de passe.")
            ...
            if keep_session:
                ...
                logout_url = res.find('a', href=re.compile('^\/settings\/security_login\/sessions\/log_out_all\/confirm'))
                if logout_url:
                    last_req = fbobj._session.get(fbobj._host + logout_url['href'])
                    return last_req.ok
                else:
                    raise exceptions.FacebookError("Erreur lors de la tentative de déconnexion de toutes les sessions.")
...
def UpdateProfilePicture(fbobj, photo):
    ...
    if c is not None:
        ...
        if c is None:
            raise exceptions.FacebookError('Tidak dapat mengganti Poto Profile')
        d = bs4(fbobj._session.get(fbobj._host + c['href']).text, 'html.parser').find('a', href=re.compile('^\/photos\/change\/profile_picture'))
        if d is None:
            raise exceptions.FacebookError("Impossible de trouver le lien pour changer la photo de profil.")
    ...
    if upload is not None:
        e = bs4(fbobj._session.get(fbobj._host + upload['href']).text, 'html.parser')

    form = e.find('form', action=re.compile('https:\/\/(z-upload\.facebook\.com|upload\.facebook\.com)'))
    if form is None:
        raise exceptions.FacebookError("Impossible de trouver le formulaire de téléchargement de photo.")
